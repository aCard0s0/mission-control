...
| `GET  …/{name}/setup` | — | the profile's `.env` and what `hermes status` makes of it; degrades to the `.env` alone when status cannot run. Each `apiKeys[]` row also carries `problem`: what `hermes auth list` holds against that key's pooled credential (`auth failed invalid_api_key (401) …`), or null. Hermes' pool outlives the `.env` line it was seeded from, so this is the one place a dead key shows — `hermes status` and the `.env` both read "not set" |
...
| `GET  …/{containerId}/auth-providers` | — | which vendors this **container** holds credentials for. Container-scoped, not per profile: credentials live in the data volume, so it answers from the `default` profile and takes no name. Each row carries `providerKey` — the `/api/providers` key that login serves (`nous`, `openai-codex`) — or null for a login the picker cannot point a profile at |
...
| Method & path | Body | Notes |
|---|---|---|
| `GET /api/providers` | — | the LLM **vendor** registry: `{ key, label, needsKey, oauth, hasCatalog, envVar }` per vendor. Compiled in, not stored — it mirrors hermes' own `CANONICAL_PROVIDERS` order and provider records, and a database row would let it drift from the CLI it has to agree with. The picker, the "needs an API key?" rule and the provider→catalog decision all read this one list. API-key OpenAI is `openai-api`, hermes' spelling since v0.21.0; the retired `openai` is accepted on every write and folded to it. `openai-codex` (ChatGPT subscription) is an OAuth row with no key and no catalog: the create dialog offers it only where the container's [auth report](#agents--hermes-profiles-inside-a-container) says it is logged in, because the login is a device flow this dashboard cannot drive |
